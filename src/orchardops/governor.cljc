(ns orchardops.governor
  "Orchard Operations Governor -- the independent compliance layer that
  earns the OrchardOpsAdvisor the right to commit. The LLM has no notion
  of:
  ...
  CRITICAL: Any proposal to flag a crop health concern (fungal disease/
  fruit-fly/frost-damage) ALWAYS escalates to a human (grower/agronomist)
  ...
  This design mirrors `vineyardops.governor` (cloud-itonami-isic-0121) but
  specializes tropical/subtropical-fruit orchard back-office coordination
  concerns (orchard/block registration, closed op allowlist,
  field-equipment/spray-application exclusion, cost threshold) rather than
  vineyard concerns."
  (:require [orchardops.facts :as facts]
            [orchardops.registry :as registry]
...
(defn- cost-threshold-for
  ...
  category-specific threshold from `orchardops.facts` if the category is
...
(defn check
  "Censors an OrchardOpsAdvisor proposal against the Governor rules.
