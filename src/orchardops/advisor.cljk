(ns orchardops.advisor
  "OrchardOpsAdvisor -- the contained LLM/decision node. This actor's
  ...
  `vineyardops.advisor`, cloud-itonami-isic-0121)."
  )
    ...
    (let [{:keys [op orchard-id]} request]
      (case op
        ...
        {:op :schedule-field-operation
         ...
         :summary "Field operation (pruning/spraying/irrigation/harvest) scheduling proposed per operator request"
         :confidence 0.85}
        ...
        {:op :flag-crop-health-concern
         ...
         :summary "Crop health concern (fungal disease/fruit-fly/frost-damage) flagged for agronomist review"
         :confidence 0.8}
