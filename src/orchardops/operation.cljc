(ns orchardops.operation
  ...
  The advisor (OrchardOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Orchard Operations
  ...
  One run = one orchard coordination operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner loop
  -- each operation is auditable and checkpointed. An orchard/block's
  operating history is advanced by MANY operations (log-orchard-record /
  ...
  Governor is clean -- see `orchardops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `vineyardops.operation`, cloud-itonami-isic-0121). This stub version
  ...
  (:require [orchardops.advisor :as advisor]
...
(defn run-operation
  "Run one orchard operation through the advisor -> governor -> phase
