(ns orchardops.phase
  "Phase gate for the orchard operations coordinator rollout (0->3
  maturity). Mirrors `vineyardops.phase` (cloud-itonami-isic-0121) in
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`orchardops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [orchardops.governor :as governor]))
